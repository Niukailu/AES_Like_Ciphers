...
//��
//declare in SPN_Best_Differential_Prob_Search.cpp
extern CNT_t mid_state_active_num;
extern std::map<std::string, std::map<std::string, PROB_t>> half_trails;
extern std::vector<RoadsByActive>* classified_left_roads;
extern std::string IntToStr(uint32_t word);
extern clock_t find_start;

//��
/*Global Variables*/
//std::map<std::pair<std::string, std::string>, std::pair<CNT_t, PROB_t>> distinguishers;
std::vector<std::pair<PROB_t, std::pair<std::string, std::string>>> meet_trails;
CNT_t Inv_target_Counter = 1 << 20;
clock_t find_end;
/*std::pair<std::string, std::string> max_distinguisher;
CNT_t max_distinguisher_include_trails_num = 0;
PROB_t max_distinguisher_prob = ZERO_PROB;*/
...
void Inv_SPN_Best_Dif_FirstRound(void);
void Inv_SPN_Best_Dif_FirstSubRound(CNT_t j, ACTIVE_MAP_t A, PROB_t Wbef);
void Inv_SPN_Best_Dif_MiddleRound(CNT_t i, PROB_t RWbef);
void Inv_SPN_Best_Dif_MiddleSubRound(CNT_t i, CNT_t j, ACTIVE_MAP_t A, PROB_t Wbef);
void Inv_SPN_Best_Dif_LastRound(PROB_t RWbef);
void Inv_SPN_Best_Dif_LastSubRound(CNT_t j, ACTIVE_MAP_t A, PROB_t Wbef);


#if defined OUR_PC1

//��
static DEV_INLINE void Prob_Fill_Undertermined(PROB_t* pred, CNT_t j, SBOX_O_WRD_t* sout, ACTIVE_MAP_t Ain)
{
	...
	/*condition*/
	// 1) i > 2
	// 2) thus, sout is determined already
	...
	if ((MAX_PROB_EQUAL_EVERY_SBOX == TRUE) && (FIX_OU_MAX_PROB_EQUAL_EVERY_SBOX == TRUE)) //This includes the case when all the S-boxes in a state equal.
	{
		//if FIX_OU_MAX_PROB_EQUAL_EVERY_SBOX is TRUE
		// We don't have to check the output value
		CNT_t HA;
	...
	{
		//We should check the output value
		for (sbox_idx = j + 1; sbox_idx < NUM_SBOX_IN_A_STATE; sbox_idx++)
		{
			if (sout[sbox_idx] != 0) //When it is not zero difference
			{
				//Multiple the maximum probability with the non-zero estimated output difference word.
				MUL_PROB(&estimated_curr_round_prob, estimated_curr_round_prob, DC_SBOXES_I_PROB_FIXED_O[sbox_idx][sout[sbox_idx]][0].p);//�� 
			}
		}
	}
...
//��
static DEV_INLINE CNT_t Prob_Next_Round_Lower_Bound(PROB_t* pred, CNT_t i, CNT_t j, SBOX_I_WRD_t* sin, ACTIVE_MAP_t A)
{
	PROB_t		  estimated_next_round_prob = ONE_PROB;
	DIFF_O_WRD_t  i_round_state_till_j_word[NUM_WORD_IN_A_STATE_THRESHOLD] = { 0, };
	DIFF_I_WRD_t  iplus1_round_partial_in[NUM_WORD_IN_A_STATE_THRESHOLD] = { 0, };
	...
	if ((DIFFUSION_TYPE == BIT_PERMUTATION) && (MAX_PROB_EQUAL_EVERY_SBOX == TRUE) && (FIX_OU_MAX_PROB_EQUAL_EVERY_SBOX == TRUE)) //This includes the case when all the S-boxes in a state equal.
	{
		CNT_t  num_active;
		PROB_t active_prob;
		...
		memcpy(i_round_state_till_j_word, sin, sizeof(DIFF_O_WRD_t) * ((size_t)num_det_word)); //��
		/*Estimated Part from Partial Determined Values*/
		Inv_Diffusion(iplus1_round_partial_in, i_round_state_till_j_word);
	...
	if (num_det_partition != 0)
	{
		...
		/*init i_round_state_till_j_word already*/
		//��
		memcpy(i_round_state_till_j_word, sin, sizeof(DIFF_O_WRD_t) * (size_t)(num_det_partition * NUM_DIFF_OF_PARTIAL_DIFFUSION));
		/*Determined Part*/
		Inv_Diffusion(iplus1_round_partial_in, i_round_state_till_j_word);
		for (sbox_idx = 0; sbox_idx < NUM_SBOX_IN_A_STATE; sbox_idx++)
		{
			...
			{
				//Multiple the maximum probability with the non-zero estimated output difference word.
				MUL_PROB(&estimated_next_round_prob, estimated_next_round_prob, DC_SBOXES_I_PROB_FIXED_O[sbox_idx][iplus1_round_partial_in[sbox_idx]][0].p);
			}
		}
	}
	...
	if (DIFFUSION_TYPE == BIT_PERMUTATION)
	{
		...
		/*With repect to estimated part*/
		memset(i_round_state_till_j_word, 0, sizeof(DIFF_O_WRD_t) * ((size_t)NUM_DIFF_IN_A_STATE)); //��
		memcpy(i_round_state_till_j_word, sin, sizeof(DIFF_O_WRD_t) * ((size_t)num_det_word));
		memset(i_round_state_till_j_word, 0, sizeof(DIFF_O_WRD_t) * ((size_t)(num_det_partition * NUM_DIFF_OF_PARTIAL_DIFFUSION)));
		/*Estimated Part from Partial Determined Values*/
		Inv_Diffusion(iplus1_round_partial_in, i_round_state_till_j_word);
		estimated_iplus1_round_active_map = Compute_Active_Map_From_State(iplus1_round_partial_in, NUM_SBOX_IN_A_STATE);
		num_active = (CNT_t)get_word_hw(estimated_iplus1_round_active_map);
...
*/
//��
void Inv_SPN_Best_DC_Prob_Search(PROB_t* prob_rst, BLK_CIPHER_INFO_t* ci, CNT_t set_round, CNT_t target_round, UFLAG_t verbose)
{
	//��
	CNT_t real_target_round = target_round;
	target_round = (target_round / 2) - 1;
	...
	{
		printf("=================Inv_SPN_Best_Differential_Trail_Search=================\n\n");
	}
	...
	std::sort(meet_trails.begin(), meet_trails.end());
	fprintf(fileptr, "The sum of the meet trails is [%u]\n\n", meet_trails.size());
	for (int i = 0; i < meet_trails.size(); i++)
	{
		fprintf(fileptr, "The meet trail [%s]-->[%s] with prob : 2^{%0.4lf}\n", meet_trails[i].second.first.c_str(), meet_trails[i].second.second.c_str(), meet_trails[i].first);
	}
	fprintf(fileptr, "===================================================================\n\n");
	fprintf(fileptr, "����ʱ��Ϊ��%f s", ((double)(find_end - find_start)) / 1000); //CLK_TCK在linux下不管用

	/*printf("The distinguishers are on the follow: \n");
	for (auto& distinguisher : distinguishers) {
		printf("The distinguisher [%s]-->[%s] with prob : 2^{%0.4lf}, including trails num : %2d\n", distinguisher.first.first.c_str(), distinguisher.first.second.c_str(), distinguisher.second.second, distinguisher.second.first);
		if (max_distinguisher_prob == ZERO_PROB || max_distinguisher_prob < distinguisher.second.second) {
			max_distinguisher = distinguisher.first;
			max_distinguisher_prob = distinguisher.second.second;
			max_distinguisher_include_trails_num = distinguisher.second.first;
		}
	}

	printf("===================================================================\n");
	printf("The distinguisher [%s]-->[%s] with max prob : 2^{%0.4lf}, including trails num : %2d\n", max_distinguisher.first.c_str(), max_distinguisher.second.c_str(), max_distinguisher_prob, max_distinguisher_include_trails_num);*/
...
void Inv_SPN_Best_Dif_FirstRound(void)
{
	...
	if (CONSTRUCT_1ROUND_ACTIVE_MAPS == FALSE)
	{
		...
		for (idx_active_map = 0; idx_active_map < NUM_1ROUND_ACTIVE_MAPS; idx_active_map++)
		{
			...
			Inv_SPN_Best_Dif_FirstSubRound(j_init, A, W_init);
		}
	}
...
void Inv_SPN_Best_Dif_FirstSubRound(CNT_t j, ACTIVE_MAP_t A, PROB_t Wbef)
{
	...
	//��
	for (_1_i = 0; _1_i < SBOX_I_CARDINALITY; _1_i++)
	{
		*X1_j = DC_SBOXES_I_O_PROB_FIXED_I[j][_1_i].i;
		//It is always active!
		if (*X1_j == (SBOX_I_WRD_t)0)
		{
			continue;
		}
		*P1_j = DC_SBOXES_I_O_PROB_FIXED_I[j][_1_i].p;
		...
		//Pruning (PC-2)
		j_nxt = Prob_Next_Round_Lower_Bound(&Wnxt, 1, j, DC_TRAIL_IN_PROGRESS[1].sub_i, A); //��
		MUL_PROB(&Wpred2, Wpred1, Wnxt);
		if (Prob_Bound_Checker(2, Wpred2) == EXCEED_BOUND)
			continue; //Try other out value

		//store the current output word
		*Y1_j = DC_SBOXES_I_O_PROB_FIXED_I[j][_1_i].o;
		...
		{
			//Move to next word
			Inv_SPN_Best_Dif_FirstSubRound(j_nxt, A, Wdet);
		}
		else //when this word is the last word
		{
			...
			if (CUR_TARGET_ROUND == 2)
			{
				...
				Inv_SPN_Best_Dif_LastRound(RWdet);
			}
			else
			{
				//Move to next(2) round
				Inv_SPN_Best_Dif_MiddleRound(2, RWdet);
			}
		}
	}
}
...
void Inv_SPN_Best_Dif_MiddleRound(CNT_t i, PROB_t RWbef)
{
	...
	//��
	Inv_Diffusion(DC_TRAIL_IN_PROGRESS[i].sub_o, DC_TRAIL_IN_PROGRESS[i - 1].sub_i);
	A = Compute_Active_Map_From_State(DC_TRAIL_IN_PROGRESS[i].sub_o, NUM_SBOX_IN_A_STATE);

	//Pruning (PC-1)
	Prob_Fill_Undertermined(&Wall, BEFORE_STARTING_THE_ROUND, DC_TRAIL_IN_PROGRESS[i].sub_o, A); //��
	...
	//init the input/output states and probabilities
	memset(DC_TRAIL_IN_PROGRESS[i].sub_i, 0, sizeof(SBOX_I_WRD_t) * (size_t)NUM_SBOX_IN_A_STATE); //��
	Set_Probs_One(DC_TRAIL_IN_PROGRESS[i].p, NUM_SBOX_IN_A_STATE);

	W_init = RWbef;

	Inv_SPN_Best_Dif_MiddleSubRound(i, j_init, A, W_init);
}
...
void Inv_SPN_Best_Dif_MiddleSubRound(CNT_t i, CNT_t j, ACTIVE_MAP_t A, PROB_t Wbef)
{
	SBOX_I_CNT_t _i_j;
	//��
	SBOX_O_WRD_t Yi_j = DC_TRAIL_IN_PROGRESS[i].sub_o[j]; //This word is fixed from before state
	SBOX_I_WRD_t* Xi_j = &(DC_TRAIL_IN_PROGRESS[i].sub_i[j]);
	PROB_t* Pi_j = &(DC_TRAIL_IN_PROGRESS[i].p[j]);
	PROB_t		 Wdet, Wnxt;
	PROB_t		 Wpred1rm, Wpred1, Wpred2;
	CNT_t		 j_nxt;

	Prob_Fill_Undertermined(&Wpred1rm, j, DC_TRAIL_IN_PROGRESS[i].sub_o, A);
	MUL_PROB(&Wpred1rm, Wbef, Wpred1rm);

	//��
	for (_i_j = 0; _i_j < NUM_I_WITH_NONZERO_PROB[j][Yi_j]; _i_j++)
	{
		*Pi_j = DC_SBOXES_I_PROB_FIXED_O[j][Yi_j][_i_j].p;
		...
		//store the current in word
		*Xi_j = DC_SBOXES_I_PROB_FIXED_O[j][Yi_j][_i_j].i;  //��

		//Pruning (PC-2)
		j_nxt = Prob_Next_Round_Lower_Bound(&Wnxt, i, j, DC_TRAIL_IN_PROGRESS[i].sub_i, A);  //��
		MUL_PROB(&Wpred2, Wpred1, Wnxt);
		if (Prob_Bound_Checker(i + 1, Wpred2) == EXCEED_BOUND)
			continue; //Try other out value
		...
		{
			//Move to next word
			Inv_SPN_Best_Dif_MiddleSubRound(i, j_nxt, A, Wdet);
		}
		else //when this word is the last word
		{
			PROB_t RWdet = Wdet;
			...
			{
				Inv_SPN_Best_Dif_MiddleRound(i + 1, RWdet);
			}
		}
	}
}
...
void Inv_SPN_Best_Dif_LastRound(PROB_t RWbef)
{
	...
	//��
	Inv_Diffusion(DC_TRAIL_IN_PROGRESS[CUR_TARGET_ROUND].sub_o, DC_TRAIL_IN_PROGRESS[CUR_TARGET_ROUND - 1].sub_i);
	A = Compute_Active_Map_From_State(DC_TRAIL_IN_PROGRESS[CUR_TARGET_ROUND].sub_o, NUM_SBOX_IN_A_STATE); //�� �����Ƿ���ݣ�

	//Pruning (PC-1)
	Prob_Fill_Undertermined(&Wall, BEFORE_STARTING_THE_ROUND, DC_TRAIL_IN_PROGRESS[CUR_TARGET_ROUND].sub_o, A);
	MUL_PROB(&Wpred1, RWbef, Wall);
	if (Prob_Bound_Checker(CUR_TARGET_ROUND, Wpred1) == EXCEED_BOUND)
		return; //We do not have to check the input sate which has more than(and equal to) the current number of active S-box

	//init the input/output states and probabilities
	memset(DC_TRAIL_IN_PROGRESS[CUR_TARGET_ROUND].sub_i, 0, sizeof(SBOX_I_WRD_t) * (size_t)NUM_SBOX_IN_A_STATE); //��
	...
	Inv_SPN_Best_Dif_LastSubRound(j_init, A, W_init);
}

void Inv_SPN_Best_Dif_LastSubRound(CNT_t j, ACTIVE_MAP_t A, PROB_t Wbef)
{
	//��
	SBOX_O_WRD_t Yn_j = DC_TRAIL_IN_PROGRESS[CUR_TARGET_ROUND].sub_o[j]; //This word is fixed from before state
	SBOX_I_WRD_t* Xn_j = &(DC_TRAIL_IN_PROGRESS[CUR_TARGET_ROUND].sub_i[j]);
	...
	//��
	SBOX_I_CNT_t _i_j;
	for (_i_j = 0; _i_j < NUM_I_WITH_NONZERO_PROB[j][Yn_j] && DONE_FLAG != TRUE; _i_j++) {

		*Pn_j = DC_SBOXES_I_PROB_FIXED_O[j][Yn_j][_i_j].p; //max prob with fixed out

		MUL_PROB(&Wdet, Wbef, *Pn_j);

		//Pruning (PC-1)
		Prob_Fill_Undertermined(&Wrm, j, DC_TRAIL_IN_PROGRESS[CUR_TARGET_ROUND].sub_o, A); //��
		MUL_PROB(&Wpred1, Wdet, Wrm);
		if (Prob_Bound_Checker(CUR_TARGET_ROUND, Wpred1) == EXCEED_BOUND)
			return;

		//store the current in word
		*Xn_j = DC_SBOXES_I_PROB_FIXED_O[j][Yn_j][_i_j].i; //��
		...
		{
			//Move to next word
			Inv_SPN_Best_Dif_LastSubRound(j_nxt, A, Wdet);
		}
		else //when this word is the last word
		{
			//��
			if (CUR_TARGET_ROUND != WHOLE_TARGET_ROUND) {
				...
				if (Verbose_Check(VERBOSE, VV_FILE_PRINT | VV_LAST_UPDATE_TRAIL) == TRUE)
				{
					...
					{
						//��
						Inv_Diffusion(DC_TRAIL_FOR_OUT[__FF__].dif_o, DC_TRAIL_FOR_OUT[__FF__].sub_i);
					}
				}
				...
				if (active_num <= mid_state_active_num) {
					...
					std::string right_tail = "";
					CNT_t __FF__;
					for (__FF__ = 0; __FF__ < NUM_SBOX_IN_A_STATE; __FF__++)
					{
						right_head += IntToStr(DC_TRAIL_FOR_OUT[1].sub_o[__FF__]);
						right_tail += IntToStr(DC_TRAIL_FOR_OUT[WHOLE_TARGET_ROUND].dif_o[__FF__]);
					}

					//�����ײ�ɹ������ظ�·��
					std::vector<Road> all_meet_trails = find_collision(right_tail, right_head, Wdet, classified_left_roads);
					
					for (auto& one_meet_trail : all_meet_trails) {
						meet_trails.push_back(make_pair(one_meet_trail.prob, make_pair(one_meet_trail.begin, one_meet_trail.end)));
						/*if (!distinguishers.count(make_pair(one_meet_trail.begin, one_meet_trail.end))) { //��ʼ��
							distinguishers[make_pair(one_meet_trail.begin, one_meet_trail.end)] = { 1, one_meet_trail.prob };
						}
						else {
							++distinguishers[make_pair(one_meet_trail.begin, one_meet_trail.end)].first;
							distinguishers[make_pair(one_meet_trail.begin, one_meet_trail.end)].second = ADD_PROB(distinguishers[make_pair(one_meet_trail.begin, one_meet_trail.end)].second, one_meet_trail.prob);
						}*/
					}
	
				}

				if (Inv_target_Counter == 0) DONE_FLAG = TRUE;
			}
			...
			if (Verbose_Check(VERBOSE, VV_UPDATE_BOUND_TRAIL) == TRUE)
			{
				...
				{
					//��
					Inv_Diffusion(DC_TRAIL_IN_PROGRESS[__FF__].dif_o, DC_TRAIL_IN_PROGRESS[__FF__].sub_i);
				}
				//��ӡ
